...
#saveRDS(metaDigitise(temp_dir, summary=F), paste0(temp_dir, "/fig6.rds"))
...
d_list <- readRDS("fig6.rds")

#### Step 1: Wrangle data ########
d <- bind_rows( d_list$`fig6. Shellfish collecting efficiency across age categories..png`  ) %>% select(id, mean, error, n)
