...
temp_dir <- "paper_data/BliegeBird_2002a/fig5a" # temporarily set directory
...
#saveRDS(metaDigitise(temp_dir, summary=F), paste0(temp_dir, "/fig5a.rds"))
...
d_list <- readRDS("fig5a.rds")

#### Step 1: Wrangle data ########
d <- bind_rows( d_list$`fig5a. Age effects on shellfish collecting efficiency. Top (women and girls).png`  ) %>% select(x, y)
...
d_fin$units <- "net kcal/hr" # all data in the paper described as "efficiency", hence net
