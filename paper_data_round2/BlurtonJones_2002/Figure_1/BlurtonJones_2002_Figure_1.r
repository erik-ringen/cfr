...
temp_dir <- "paper_data_round2/BlurtonJones_2002/Figure_1" # temporarily set directory

### Pre-lim: digitize figure data
#metaDigitise(temp_dir)

# workflow: no groups needed, just select all points on the scatter
...
#### Step 1: Wrangle data ########
d <- bind_rows( d_list$scatterplot  ) %>% select(  
  x, y)
